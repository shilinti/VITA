...
if ! cd ../HybrIK; then
    echo "Directory 'HybrIK' does not exist."
    exit 1
fi
...
wget --post-data "username=$username&password=$password" 'https://download.is.tue.mpg.de/download.php?domain=smplify&sfile=mpips_smplify_public_v2.zip&resume=1' -O '~/projects/vita/HybrIK/mpips_smplify_public_v2.zip' --no-check-certificate --continue
